...
const PokemonList = () => {
  ...
  return (
    <>
      <Toaster/>
      <h1 style={{fontSize: '5vw', fontFamily: 'Pokemon Solid, sans-serif', marginBottom: '5%', color: '#ffcb05'}}
          className={"text-center"}>My Pokémon</h1>
      <div className={"d-flex flex-column justify-content-center align-items-center gap-4"}>
        ...
        <Link style={{width: "20vw", fontSize: "1vw"}} className={"btn btn-warning"} to={{pathname: "/my-pokedex"}}><img
          className={"pe-3"} style={{minWidth: "50px", width: "20%"}}
          src={"https://www.freeiconspng.com/thumbs/pokeball-png/free-pokeball-download-3.png"} alt={"pokeball"}/>Accéder
          au pokédex personnel</Link>
        <div className={"d-flex gap-3 flex-wrap justify-content-center align-items-center"}>
          {displayedPokemon.map((pokemon, index) => (
            <div key={index} className={"card pokemon-dislike-button"}
              ...
              <div className={"card-body d-flex flex-column align-items-center justify-content-center"}>
                ...
                <button type={'button'} className={"btn btn-success"} onClick={addToPokedex(pokemon)}>Ajouter au
                  Pokédex
                </button>
              </div>
            </div>
        ...
        {totalPages > 1 && (
          <div className="navigation-buttons">
            <button className={"position-fixed rounded-circle z-1 btn btn-secondary"}
                    style={{left: '2%', top: '50%', transform: 'translate(-50%, -50%)', width: '50px', height: '50px'}}
                    onClick={handlePrevPage} disabled={currentPage === 0}>
              <FontAwesomeIcon icon={faChevronLeft}/>
            </button>
            <button className={"position-fixed rounded-circle z-1 btn btn-secondary"}
                    style={{right: '2%', top: '50%', transform: 'translate(50%, -50%)', width: '50px', height: '50px'}}
                    onClick={handleNextPage} disabled={currentPage >= totalPages - 1}>
